...
import {
  fetchFavoriteFundsRequest,
  removeFavoriteFundRequest,
} from "../../redux/actions/favoriteFundsActions";
import styles from "./index.module.scss";

const FavoriteFunds: React.FC = () => {
  const dispatch = useAppDispatch();
  const { list, loading, error } = useAppSelector(
    (state) => state.favoriteFunds
  );
  ...
  return (
    <div className={styles["favorite-funds-container"]}>
      <Card
        className={styles["favorite-funds-card"]}
        title="自选基金"
        loading={loading}
      >
        {error && (
