...
const TransactionHistory: React.FC = () => {
  ...
  const transactionsColumns = [
    ...
    },
    {
      ...
      width: 100,
      render: (type: string) => (
        <span className={type === 'purchase' ? styles['purchase-type'] : styles['redeem-type']}>
          {type === 'purchase' ? '买入' : '卖出'}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (status: string) => (
        <span className={status === 'completed' ? styles['completed-status'] : styles['pending-status']}>
  ...
  return (
    <div className={styles['transaction-history-container']}>
      <Card className={styles['transaction-history-card']} title="交易记录" loading={loading}>
        <Table
